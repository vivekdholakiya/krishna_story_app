...
//     );
//
//     // Diagnostic: confirms whether the manifest actually has a Hindi
//     // entry for this story on this platform at runtime.
//     if (kDebugMode) {
//       final entry = AudioManifest.instance.entryFor(widget.storyKey);
//       debugPrint(
//         '[StoryDetail] storyKey=${widget.storyKey} lang=$selectedLanguage '
//             'platform=${Platform.operatingSystem} hasEntry=${AudioManifest.instance.hasEntry(widget.storyKey)} '
...
//       if (kDebugMode) {
//         debugPrint(
//           '[StoryDetail] preload OK storyKey=${widget.storyKey} '
//               'path=${_preloadedFile?.path} exists=${_preloadedFile != null ? await _preloadedFile!.exists() : false}',
//         );
//       }
//     } catch (e, st) {
//       // Was previously swallowed with catch (_) — now logged so platform-
//       // specific failures (e.g. iOS-only Hindi audio) are visible.
//       debugPrint(
//         '[StoryDetail] preload FAILED storyKey=${widget.storyKey} '
//             'lang=$selectedLanguage platform=${Platform.operatingSystem} '
//             'error=$e',
//       );
...
//       if (kDebugMode) {
//         debugPrint('[StoryDetail] TTS error storyKey=${widget.storyKey} msg=$msg');
//       }
//       if (!mounted) return;
//       _ttsProgressTimer?.cancel();
...
//     });
//
//     // Surfaces low-level just_audio errors (network/codec/ATS-type failures)
//     // that would otherwise only show up as a generic snackbar.
//     _player.playbackEventStream.listen(
//           (event) {},
//       onError: (Object e, StackTrace st) {
//         debugPrint(
//           '[StoryDetail] player stream ERROR storyKey=${widget.storyKey} '
//               'lang=$selectedLanguage platform=${Platform.operatingSystem} error=$e',
//         );
//         if (kDebugMode) debugPrintStack(stackTrace: st);
//         if (mounted) {
...
//       if (kDebugMode) {
//         debugPrint(
//           '[StoryDetail] no manifest entry storyKey=${widget.storyKey} '
//               'lang=$selectedLanguage platform=${Platform.operatingSystem} → TTS fallback',
//         );
//       }
...
//         );
//       } catch (e, st) {
//         // Was previously swallowed with catch (_) — now logged with full
//         // detail so platform-specific failures (iOS Hindi) are visible.
//         debugPrint(
//           '[StoryDetail] getAudio() THREW storyKey=${widget.storyKey} '
//               'lang=$selectedLanguage platform=${Platform.operatingSystem} '
//               'error=$e',
//         );
//         if (kDebugMode) debugPrintStack(stackTrace: st);
...
//     if (file == null) {
//       debugPrint(
//         '[StoryDetail] audio file is NULL after fetch storyKey=${widget.storyKey} '
//             'lang=$selectedLanguage platform=${Platform.operatingSystem} '
//             'preloadDone=$_preloadDone preloadedFileWasNull=${_preloadedFile == null}',
//       );
...
//     }
//
//     // Confirm the file actually exists and has bytes before handing it to
//     // the player — a common iOS-specific failure is a File object that
//     // points at a path the sandbox never actually wrote to.
//     final exists = await file.exists();
//     final length = exists ? await file.length() : 0;
//     if (kDebugMode) {
//       debugPrint(
//         '[StoryDetail] resolved audio file storyKey=${widget.storyKey} '
//             'path=${file.path} exists=$exists length=$length cacheHit=$_lastFetchWasCacheHit',
//       );
//     }
//     if (!exists || length == 0) {
//       debugPrint(
//         '[StoryDetail] audio file INVALID storyKey=${widget.storyKey} '
//             'lang=$selectedLanguage platform=${Platform.operatingSystem} '
//             'path=${file.path} exists=$exists length=$length',
//       );
//       _audioAnalytics.downloadFailed(
//         errorType: 'invalid_file',
//         errorDetail: 'exists=$exists length=$length',
//       );
//       if (mounted) setState(() => _isLoading = false);
//       _showErrorSnackbar();
//       return;
...
//     } catch (e, st) {
//       debugPrint(
//         '[StoryDetail] setFilePath/play FAILED storyKey=${widget.storyKey} '
//             'lang=$selectedLanguage platform=${Platform.operatingSystem} '
...
//     } catch (e, st) {
//       debugPrint(
//         '[StoryDetail] TTS speak() FAILED storyKey=${widget.storyKey} '
//             'lang=$selectedLanguage platform=${Platform.operatingSystem} error=$e',
//       );
//       if (kDebugMode) debugPrintStack(stackTrace: st);
//       if (mounted) setState(() => _isLoading = false);
...
class _StoryDetailScreenState extends State<StoryDetailScreen>
    with WidgetsBindingObserver {
  // ── Audio (audioplayers + TTS fallback) ─────────────────────
  final AudioPlayer _player = AudioPlayer();
  ...
  void dispose() {
    ...
    if (_isPlaying) {
      _audioAnalytics.skipped(
        positionSec: _elapsed.inSeconds,
        durationSec: _totalDuration.inSeconds,
      );
    }
  ...
  void _initPlayerStreams() {
    ...
    // PlayerState stream — audioplayers exposes stopped/playing/paused/
    // completed/disposed rather than just_audio's processingState+playing
    // pair, so we derive the loading flag from the state transitions here.
    ...
    // Completion stream — fires once when playback reaches the end.
  ...
  Future<void> _togglePlayback() async {
    ...
    if (_isPlaying) {
      _audioAnalytics.paused(positionSec: _elapsed.inSeconds);
      if (_usingTts) {
        await _tts.stop();
        _ttsProgressTimer?.cancel();
      } else {
    ...
    // Resume from paused audioplayers playback
    if (!_usingTts &&
        _player.state == PlayerState.paused &&
        _elapsed > Duration.zero &&
        _elapsed < _totalDuration) {
      _audioAnalytics.resumed(positionSec: _elapsed.inSeconds);
  ...
  Future<void> _loadAndPlay() async {
    ...
    try {
      // play() with a DeviceFileSource loads + starts playback; loading/
      // playing state is then driven by onPlayerStateChanged.
      ...
      if (mounted) {
        ...
        // _isLoading / _isPlaying are driven by onPlayerStateChanged from here.
      }
    } catch (e, st) {
      debugPrint(
        '[StoryDetail] play() FAILED storyKey=${widget.storyKey} '
            'lang=$selectedLanguage platform=${Platform.operatingSystem} '
            'path=${file.path} error=$e',
      );
      if (kDebugMode) debugPrintStack(stackTrace: st);
